...
class TransformersAndEstimators extends wp.Spec with SharedSparkContext {
  ...
  "example - tokenizer" in {
    ...
    val sentenceDataFrame0: DataFrame = sqlContext.createDataFrame(Seq(
  ...
  "example estimator" in {
    ...
    val sqlContext = new SQLContext(sc)
    val trainings = sqlContext.createDataFrame(Seq(
