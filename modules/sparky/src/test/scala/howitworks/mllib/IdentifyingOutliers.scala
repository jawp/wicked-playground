...
class IdentifyingOutliers extends wp.Spec with SharedSparkContext {
  ...
  "it must work" in {
    ...
    val dfOutliers = dfVec.select("id", "features").unionAll(sqlContext.createDataFrame(Seq(
      (10, Vectors.dense(3.0, 3.0, 3.0))
    )))
    ...
    val rddVec: RDD[Vector] = dfScaled.rdd.map(_(1).asInstanceOf[org.apache.spark.mllib.linalg.Vector])

    //this returns matrix type of mllib.linalg.Matrix
    val colCov: mllib.linalg.Matrix = Statistics.corr(rddVec)
